...
// 		module version
// 		module type
// 		settings max count in bytes
// 		name max size in bytes
// 		can requst id
// 		can reply id
// 		can setup id
void SETTINGS::begin(SETTINGS_CONFIG config) {

// CAN_COM* _can_com, uint16_t version, uint8_t type, uint8_t count, uint8_t name_size, uint16_t request, uint16_t reply, uint16_t setup) {

	_can_com = config.can_com;

	_max_count = config.settings_count;
	_declaration = new SETTINGS_TYPE[_max_count];

	_max_name_size = config.name_size;
	...
	_request_id = config.request_id;
	_reply_id = config.reply_id;
	_setup_id = config.setup_id;

	_version = config.version; // module version
	_type = config.module_type; // module type
...
// send a settings heartbeat
// CAN-ID: CAN_ID_REPLY
// byte 1: settings count
// byte 2: module version
// byte 3: module type id
void SETTINGS::heartbeat(void) {

	// heardbeat is defined
	if (_heartbeat != NULL) {

		// heartbeat timeout
		// send message
		if (_heartbeat->update()) {
			_sendHeartbeat();
		}
	}
}
...
// send heartbeat
void SETTINGS::_sendHeartbeat(void) {

	_buffer[0] = _setting_count;
	_buffer[1] = _version >> 8;
	_buffer[2] = _version & 0x0F;
	_buffer[3] = _type;

	_can_com->send(_buffer, 4, _reply_id);
}
